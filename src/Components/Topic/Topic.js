...
const Topic = ({ topic }) => {
  ...
  return (
    <div className="bg-slate-200 rounded-md shadow-lg">
      <div>
        <img className="p-2 rounded-md shadow-sm bg-slate-300 w-full" src={logo} alt="" />
      </div>
      <div className="flex justify-between mx-2 my-2">
        ...
        <div className="flex items-center sm:mr-3">
          <Link to={`/quiz/${topic.id}`}>
            <button className="bg-blue-600 shadow-lg px-1 sm:px-3 font-light sm:py-1 text-slate-300 rounded-sm">
              Go on
            </button>
          </Link>
        </div>
